...
def dtm():
    docs = [helper.read_file('article1.txt'), helper.read_file('article2.txt'), helper.read_file('article3.txt')]
    vec = CountVectorizer()
    return pandas.DataFrame(vec.fit_transform(docs).toarray(), columns=vec.get_feature_names())

def tf():
    first_sentence = helper.filter_sentence(helper.read_file('article1.txt').split(" "))
    second_sentence = helper.filter_sentence(helper.read_file('article2.txt').split(" "))
    third_sentence = helper.filter_sentence(helper.read_file('article3.txt').split(" "))
